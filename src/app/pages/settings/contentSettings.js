...
const ContentRevisions = ({ setError, notify }) => {
	...
	const contentRevisionsNoticeTitle = () => {
		return (
			__('Post revision setting saved ', 'wp-plugin-blueprint')
		);
	};
	...
	const contentRevisionsNoticeText = () => {
		return (
			...
			contentRevisions +
			_n(' revision.', ' revisions.', parseInt(contentRevisions), 'wp-plugin-blueprint')
		);
	};

	const contentRevisionsDescriptionText = () => {
		return (
			__('Saving drafts and updating published content creates revisions. Make changes with confidence, knowing you can take ', 'wp-plugin-blueprint') +
			contentRevisions +
			_n(' step back.', ' steps back.', parseInt(contentRevisions), 'wp-plugin-blueprint')
		);
	};

	const handleContentRevisionsChange = (value) => {
		blueprintSettingsApiFetch({ contentRevisions: value }, setError, (response) => {
	...
	return (
		<SelectField
			...
			label={__('Number of revisions posts can save ', 'wp-plugin-blueprint')}
...
const EmptyTrash = ({ setError, notify }) => {
	...
	const emptyTrashNoticeTitle = () => {
		return (
			__('Trash setting saved ', 'wp-plugin-blueprint')
		);
	};

	const emptyTrashNoticeText = () => {
		return (
			__('The trash will automatically empty every ', 'wp-plugin-blueprint') +
			numTrashWeeks +
			_n( ' week.', ' weeks.', parseInt(numTrashWeeks), 'wp-plugin-blueprint' )
		);
	};

	const handleEmptyTrashDaysChange = (value) => {
		blueprintSettingsApiFetch({ emptyTrashDays: value }, setError, (response) => {
	...
	return (
		<SelectField
			...
			label={__('Trash emptying frequency ', 'wp-plugin-blueprint')}
			description={
			__('The trash will automatically empty every ', 'wp-plugin-blueprint') +
			numTrashWeeks +
			 _n( ' week.', ' weeks.', parseInt(numTrashWeeks), 'wp-plugin-blueprint' )
...
const ContentSettings = () => {
	...
	return (
		<SectionSettings
			title={__('Content Options', 'wp-plugin-blueprint')}
			description={__('Controls for content revisions and how often to empty the trash.', 'wp-plugin-blueprint')}
		>
